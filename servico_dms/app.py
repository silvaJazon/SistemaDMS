...
# 3. Fornecer uma API (/api/config) para calibração (incluindo rotação).
...
JPEG_QUALITY = 50 
# (NOVO) Lê a rotação inicial; o 'camera_thread' vai usá-la
...
def get_config():
    ...
    try:
        if dms_monitor and cam_thread:
            # 1. Obtém as configurações do Dlib
            dms_settings = dms_monitor.get_settings()
            
            # 2. Obtém as configurações da Câmara
            dms_settings['brightness'] = cam_thread.get_brightness()
            dms_settings['rotation'] = cam_thread.get_rotation() # (NOVO)
...
def set_config():
    ...
    try:
        ...
        # 1. Atualiza as configurações do Dlib
        if dms_monitor:
            dms_monitor.update_settings(data)

        # 2. Atualiza as configurações da Câmara
        if cam_thread:
            if 'brightness' in data:
                cam_thread.update_brightness(data.get('brightness'))
            # (NOVO) Atualiza a rotação
            if 'rotation' in data:
                cam_thread.update_rotation(data.get('rotation'))
...
if __name__ == '__main__':
    try:
        ...
        # (NOVO) Passa a rotação inicial lida da variável de ambiente
        cam_thread = CameraThread(
            VIDEO_SOURCE, 
            FRAME_WIDTH_DISPLAY, 
            FRAME_HEIGHT_DISPLAY, 
            initial_rotation=ROTATE_FRAME
        )
        cam_thread.start()
