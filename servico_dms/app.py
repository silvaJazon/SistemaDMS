...
def generate_video_stream():
    ...
    # ... (igual a antes, incluindo logs DEBUG)
...
if __name__ == '__main__':
    ...
    try:
        ...
        while cam_thread.get_frame() is None and cam_thread.is_alive():
             if stop_event.wait(timeout=0.2): raise SystemExit("Encerrado init câmara.")
             if time.time() - start_wait_cam > 15: raise RuntimeError("Timeout câmara.")
        if not cam_thread.is_alive(): raise RuntimeError("Thread câmara terminou.")
        ...
        logging.info(f">>> A iniciar servidor web porta 5000...")
        if HAS_WAITRESS:
            logging.info("A usar Waitress.")
            serve(app, host='0.0.0.0', port=5000, threads=8)
        else:
            logging.warning("Waitress não encontrado. Usar `pip install waitress`.")
            logging.warning("A usar Flask dev server (NÃO P/ PRODUÇÃO).")
            try:
        ...
        for t in threads_to_join:
             ...
             if t.is_alive(): logging.warning(f"!!! Timeout thread '{t.name}'.")
