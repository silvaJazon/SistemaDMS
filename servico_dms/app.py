...
# (Atualizado com API para pitch configurável)
...
event_handler = None
event_queue = None
dms_monitor = None # Inicializa dms_monitor como None
...
def detection_loop(cam_thread_ref, dms_monitor_ref, event_queue_ref):
    ...
    while not stop_event.is_set():
        ...
        if not cam_thread_ref or not cam_thread_ref.is_alive(): # Usa ref
             logging.error("!!! Thread da câmara não está ativa. A parar loop de deteção.")
             break

        logging.debug("DetectionLoop: A chamar cam_thread.get_frame()...")
        frame = cam_thread_ref.get_frame() # Usa ref

        if frame is None:
            if not stop_event.is_set(): logging.debug("Frame não recebido. A aguardar...")
            stop_event.wait(timeout=0.1) # Usa wait em vez de sleep
            continue
        logging.debug("DetectionLoop: cam_thread.get_frame() retornou um frame.")

        try:
            logging.debug("DetectionLoop: A converter frame para cinzento...")
            gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)

            logging.debug("DetectionLoop: A chamar dms_monitor.process_frame()...")
            # Verifica se dms_monitor_ref foi inicializado
            if dms_monitor_ref is None:
                logging.error("!!! dms_monitor_ref não inicializado no detection_loop!")
                stop_event.wait(timeout=1.0)
                continue

            processed_frame, events, status_data = dms_monitor_ref.process_frame(frame.copy(), gray) # Usa ref
            logging.debug("DetectionLoop: dms_monitor.process_frame() retornou.")
            ...
            frame_count += 1
            if frame_count % 100 == 0: logging.debug(f"Loop deteção: Frame {frame_count} processado.")
            ...
            if events:
             ...
             logging.error(f"Erro OpenCV no process_frame (shape: {frame.shape if frame is not None else 'None'}): {cv_err}", exc_info=True)
             stop_event.wait(timeout=1.0)
        ...
        logging.debug(f"Tempo processamento: {processing_time:.3f}s, Espera: {max(0, wait_time):.3f}s")
        ...
        if wait_time > 0:
            ...
            logging.debug("DetectionLoop: Loop lento, pequena pausa (0.01s).")
...
def generate_video_stream():
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            if not flag:
                logging.warning(f"generate_video_stream: Falha ao codificar (placeholder={use_placeholder}). Tentando placeholder.")
                (flag, encodedImage) = cv2.imencode(".jpg", placeholder, [int(cv2.IMWRITE_JPEG_QUALITY), 50])
                if not flag:
                     logging.error("generate_video_stream: Falha ao codificar placeholder. A saltar frame.")
            ...
            frame_yield_count += 1
             ...
             logging.error(f"generate_video_stream: Erro OpenCV ao codificar (ph={use_placeholder}, shape={frame_to_encode.shape}): {cv_err}", exc_info=True)
             stop_event.wait(timeout=0.5)
        except Exception as e:
             logging.error(f"generate_video_stream: Erro inesperado: {e}", exc_info=True)
             break

        target_stream_time = 1/20 # Stream a 20 FPS
        current_time = time.time()
        sleep_time = target_stream_time - (current_time - last_frame_time)
        if sleep_time > 0: stop_event.wait(timeout=sleep_time)
        last_frame_time = time.time()

    logging.info(f"generate_video_stream: Terminado após enviar {frame_yield_count} frames.")
...
def video_feed():
    ...
    if not cam_thread or not cam_thread.is_alive():
         logging.error("Rota /video_feed: Thread da câmara não ativa.")
...
def api_config():
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            # (NOVO) Passa todo o dicionário new_settings para update_settings
            ...
            try:
                ...
                if 'rotation' in new_settings: cam_thread.update_rotation(new_settings['rotation'])
                 ...
                 logging.error(f"Erro ao atualizar conf. câmara: {e}")
                 cam_success = False
...
def api_alerts():
    ...
    if not event_handler:
         logging.warning("/api/alerts: Gestor de eventos não inicializado.")
    ...
    try:
        ...
        logging.error(f"Erro ao ler alertas SQLite: {e}", exc_info=True)
...
def serve_alert_image(filepath):
    ...
    try:
        ...
        logging.error(f"Erro ao servir imagem '{filepath}': {e}", exc_info=True)
...
if __name__ == '__main__':
    ...
    try:
        logging.info(f">>> Serviço DMS (com Bocejo + SQLite) a iniciar... (Log: {logging.getLevelName(logging.getLogger().level)})")
        ...
        while cam_thread.get_frame() is None and cam_thread.is_alive():
             if stop_event.wait(timeout=0.2): raise SystemExit("Encerrado durante init câmara.")
             if time.time() - start_wait_cam > 15: raise RuntimeError("Timeout espera câmara.")
        if not cam_thread.is_alive(): raise RuntimeError("Thread câmara terminou inesperadamente.")
        logging.info(">>> Primeiro frame recebido!")

        detection_thread = threading.Thread(target=detection_loop,
                                            args=(cam_thread, dms_monitor, event_queue), # Passa dms_monitor
        ...
        if HAS_WAITRESS:
            ...
            logging.warning("Pacote 'waitress' não encontrado. Usar `pip install waitress`.")
            logging.warning("A usar servidor Flask dev (NÃO PARA PRODUÇÃO).")
            try:
                app.run(host='0.0.0.0', port=5000, debug=False, threaded=True, use_reloader=False)
            except OSError as e:
                 logging.error(f"!!! ERRO FATAL ao iniciar Flask: {e}", exc_info=True)
                 stop_event.set()
            except Exception as e:
                 logging.error(f"!!! ERRO FATAL no Flask: {e}", exc_info=True)
                 stop_event.set()
        ...
        for t in threads_to_join:
             ...
             timeout = 2 if t.daemon else 5
             t.join(timeout=timeout)
             if t.is_alive(): logging.warning(f"!!! Timeout ao esperar thread '{t.name}'.")
